...
impl Grid {
    ...
    fn from_file<P>(input: P) -> Result<Self, Box<dyn Error>>
    where
        P: AsRef<Path> + Clone,
    {
...
fn main() {
    ...
    let grid = Grid::from_file(input).unwrap_or_else(|err| {
        eprintln!("error: {}: {}", input, err);
        std::process::exit(3);
    });
    println!("{}", part1::solve(grid.clone()));
    println!("{}", part2::solve(grid));
}
