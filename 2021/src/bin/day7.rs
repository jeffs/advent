...
fn load_crabs<P: AsRef<Path>>(input: P) -> Result<Vec<i32>, Box<dyn Error>> {
...
fn main() {
    ...
    let crabs = load_crabs("tests/day7/input").unwrap_or_else(|err| {
        eprintln!("error: {}: {}", input, err);
        std::process::exit(3);
    });
    println!("{}", part1::solve(&crabs));
    println!("{}", part2::solve(&crabs));
}
