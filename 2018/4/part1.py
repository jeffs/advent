...
# Map guards to lists of naps
# Find guard with most naptime (sum of lengths of naps)
# For that guard, for each nap, for each minute, incrementer per-minute counter
# Find minute with greatest count
if __name__ == '__main__':

    nap_dict = read_records('input')    # guard -> naps

    nap_length_dict = {                 # guard -> total nap time
        guard_id: sum(map(get_nap_length, naps))
        for guard_id, naps in nap_dict.items()
    }

    sleepiest_guard_id = max_by_value(nap_length_dict)

    minute_counts = count_minutes(nap_dict[sleepiest_guard_id])
    sleepiest_time = max_by_value(minute_counts)

    print(sleepiest_guard_id * sleepiest_time[1])
